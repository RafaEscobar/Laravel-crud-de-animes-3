<div>
    ...
    <div class="mb-6">
        ...
        {{ Form::text('name', $user->name ?? old('name'), ['class' => 'border-2 border-gray-300 rounded-lg pl-2 focus:outline-none focus:border-blue-400 w-full']) }}
    </div>
    <div class="mb-10">
        {{ Form::label('email', 'Ingresa el correo electrónico del usuario:', ['class' => 'text-lg']) }}
        {{ Form::email('email', $user->email ?? old('email'), ['class' => 'border-2 border-gray-300 rounded-lg pl-2 focus:outline-none focus:border-blue-400 w-full', 'id' => 'email']) }}
    </div>
    <div class="mb-10">
    ...
    <div class="flex">
        ...
        <div id="divImage" class="flex justify-center items-center w-1/2">
            @if (!empty($user))
                <img src="{{ Storage::url($user->profile_photo_path) }}" alt="" id="destinationImage" class="w-40 h-40 rounded-full">
            @else
